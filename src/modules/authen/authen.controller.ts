...
export class AuthenController {
  ...
  @ApiOperation({
    ...
    description: 'Create user for further using.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiOperation({
    ...
    description:
      'Provides a new access token using a valid refresh token from cookie.',
  })
  ...
  async refreshToken(@Req() req, @Res({ passthrough: true }) res) {
    ...
    try {
      const result = await this.authenService.refreshToken(
        user_id,
        refreshToken,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Returns minimal user info if the JWT cookie is valid and received.',
  })
  @ApiResponse({ status: 200, description: 'User is currently authenticated.' })
  @ApiResponse({
    status: 401,
    description: 'User is not authenticated (no valid cookie).',
  })
  ...
  @ApiOperation({
    summary: 'User logout',
    description: 'Withdraw JWT token and logout',
  })
  ...
  @ApiOperation({
    ...
    description: 'Check email and send url to verify email.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async verifyEmail(@Request() req, @Body('email') email: string) {
    ...
    if (status === false)
      throw new ConflictException('Email has been used by other fluffizen');
    return;
  }
  ...
  @ApiResponse({
    status: 400,
    description: 'Email already binded for other account',
  })
  @ApiResponse({ status: 409, description: 'Token invalid/expired' })
  @Get('verify')
  async acceptVerifyEmail(@Query('token') token: string) {
    ...
    if (status === false)
      throw new ConflictException('Email already binded for other account');
    return;
  }
  ...
  async unbindEmail(@Request() req) {
    ...
    if (status === false)
      throw new ConflictException(`User don't have email binded`);
    return;
  }
}
...
export class AdminController {
  constructor(private readonly adminService: AdminAuthenService) {}

  // For Admin and Super Admin
  @ApiOperation({
    summary: 'Admin get user info',
    description: 'Get user info by username or email.',
  })
  @Post('get-user')
  ...
  @ApiOperation({
    ...
    description: 'Ban user by username or email.',
  })
  @Post('ban')
  async banUser(@Req() req, @Body() body: { user_id: number; reason: string }) {
    const { user_id, reason } = body;
    const status = await this.adminService.banUser(
      user_id,
      req.user.role,
      reason,
    );
  ...
  @ApiOperation({
    ...
    description: 'Unban user by username or email.',
  })
  @Post('unban')
  ...
  @ApiOperation({
    ...
    description: 'Suspend user by user ID.',
  })
  @Post('suspend')
  async suspendUser(
    @Req() req,
    @Body()
    body: { user_id: number; till: Date; reason: string },
  ) {
    const { user_id, till, reason } = body;
    const status = await this.adminService.suspendUser(
      user_id,
      req.user.role,
      till,
      reason,
  ...
  @ApiOperation({
    ...
    description: 'Unsuspend user by user ID.',
  })
  @Post('unsuspend')
  async unsuspendUser(@Req() req, @Body('user_id') user_id: number) {
    const status = await this.adminService.unsuspendUser(
      user_id,
      req.user.role,
    );
    if (status === null) throw new BadRequestException('User not found');
  ...
  @ApiOperation({
    ...
    description: 'Verify user by username or email.',
  })
  @Post('verify')
...
export class SuperAdminController {
  constructor(private readonly superAdminService: SuperAdminAuthenService) {}

  // For Super Admin only
  @ApiOperation({
    summary: 'Super Admin create new admin',
  ...
  @ApiOperation({
    summary: 'Super Admin change role for account',
    description: 'Change role for account',
  })
  @Post('change-role')
  async changeRole(@Req() req, @Body('user_id') user_id: number) {
    const status = await this.superAdminService.changeRole(
      user_id,
      req.user.role,
    );
    if (status === null) throw new BadRequestException('User not found');

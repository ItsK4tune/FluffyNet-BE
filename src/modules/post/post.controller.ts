...
export class PostController {
  ...
  @ApiOperation({ summary: 'Get total number of posts by user ID' })
  @ApiParam({ name: 'user_id', type: Number })
  @ApiResponse({ status: 200, description: 'Total number of posts.' })
  @Get('count/:user_id')
  async getPostCountByUser(
    @Param('user_id', ParseIntPipe) user_id: number,
  ): Promise<{ total: number }> {
    const total = await this.postService.countPostsByUser(user_id);
    return { total };
  }
}

...
export class NotificationController {
  ...
  @ApiOperation({
    summary: `Get all user's notification or only unread ones`,
    description: `Pass ?unreadOnly=true to filter unread notifications`,
  })
  ...
  async markAllAsOpened(@Req() req) {
    try {
      const user_id = req.user.user_id;
      const result = await this.notificationService.markAllAsOpen(user_id);
      return {

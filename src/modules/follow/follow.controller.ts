...
export class FollowController {
  ...
  @ApiOperation({
    summary: `Get status whether user has follow target`,
    description: `Authenticate, authorize and return user's following status toward target.`,
  })
  ...
  @ApiOperation({
    summary: `Set follow status from user to target`,
    description: `Authenticate, authorize and set user's following status toward target.`,
  })
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'user')
  @ApiBearerAuth()
  ...
  @ApiOperation({
    summary: `Get all target following`,
    description: `Get target's following list.`,
  })
  @UseGuards(RolesGuard)
  @Roles('admin', 'user')
  @ApiResponse({
    status: 200,
    description: 'Following list of user_id: <user_id> + following list',
  })
  @ApiResponse({ status: 400, description: 'User not found' })
  @Get('following')
  async getFollowingList(@Query('user_id') user_id: number) {
    ...
    return {
      message: `Following list of user_id: ${user_id}`,
      statusCode: 200,
      list: list,
    };
  }
  ...
  @ApiOperation({
    summary: `Get all target follower`,
    description: `Get target's follower list.`,
  })
  @UseGuards(RolesGuard)
  @Roles('admin', 'user')
  @ApiResponse({
    status: 200,
    description: 'Follower list of user_id: <user_id> + follower list',
  })
  @ApiResponse({ status: 400, description: 'User not found' })
  @Get('follower')
  async getFollowerList(@Query('user_id') user_id: number) {
    ...
    return {
      message: `Follower list of user_id: ${user_id}`,
      statusCode: 200,
      list: list,
    };
  }

...
import { CommentUploadCompleteDto, CommentUploadPresignDto } from './dtos/comment-upload.dto'
...
export class CommentController {
  ...
  @ApiParam({ name: 'post_id', description: 'ID of the post to get comments for' })
  ...
  @Get('/post/:post_id') 
  async getCommentsByPost(@Param('post_id', ParseIntPipe) post_id: number): Promise<{ comments: any[] }> { 
  ...
  async createComment(
    ...
    try {
      const newComment = await this.commentService.createComment(post_id, userId, commentDto);
  ...
  @ApiOperation({ summary: 'Get presigned URL for uploading comment image/video' })
  @ApiResponse({ status: 201, description: 'Presigned URL generated.', schema: { type: 'object', properties: { presignedUrl: { type: 'string' }, objectName: { type: 'string' } } } })
  @ApiResponse({ status: 400, description: 'Invalid input (filename, contentType).'})

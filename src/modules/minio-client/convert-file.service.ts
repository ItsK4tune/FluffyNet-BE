...
export class ConvertFileService {
    ...
    async convertMp4ToHls(category: string, user_id: number, post_id: number, objectName: string, seekTimeThumbnail: number = 5): Promise<void> {
        ...
        const targetHlsPrefix = `${category}/user_${user_id}/hlses/post_${post_id}/`;
        ...
        try {
            ...
            if (!fs.existsSync(tmpInput) || fs.statSync(tmpInput).size === 0) {
                throw new InternalServerErrorException('Downloaded video file is missing or empty.');
            }
            ...
            await new Promise<void>((resolve, reject) => {
                ffmpeg(tmpInput)
                    .addOptions([
                        ...
                        '-f hls',
                        '-preset veryfast', 
                        '-hls_segment_filename', path.join(tmpOutputDirHls, 'segment%03d.ts') 
                    ])
            ...
            const thumbnailObjectName = `${targetHlsPrefix}thumbnail.jpg`;
            ...
            for (const file of files) {
                ...
                const targetObjectName = `${targetHlsPrefix}${file}`;
                ...
                try {
                    ...
                    throw new InternalServerErrorException(`Lỗi khi upload file HLS: ${file}`, uploadError.message);
                }
            }
        } catch (error: any) {
            throw error instanceof Error ? error : new InternalServerErrorException('Lỗi không xác định trong quá trình chuyển đổi video.');
        } finally {
            try {
